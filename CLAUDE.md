...
```
FileObjectId/
    FileObjectId.psm1        # Main module code (P/Invoke + functions)
...
Export-CrescendoModule -ConfigurationFile ./FileObjectId/Fsutil.Crescendo.json -ModuleName ./FileObjectId/Fsutil.psm1 -Force
```
...
- **Pester mocking scope.** Tests must use `-ModuleName FileObjectId` on
...
Bump `ModuleVersion` in `FileObjectId.psd1` on any user-visible change.
